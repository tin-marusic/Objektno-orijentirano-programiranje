...
class Sprite extends GameWorldObjectAnimator {
  ...
  updateAnimation() {
    ...
    if (this.jumping === true && this.penjanje_skale === false) {
    ...
    // ako je lik okrenut desno
      ...
      if (this.velocity_x > 0.1) this.changeFrameSet(this.frameSets("walk-right"), "loop", 3);
      // ako stoji, onda prikaži zadani položaj za desno
      else this.changeFrameSet(this.frameSets("right"), "pause");
    }
    else if (this.direction == 270) {
      if (this.velocity_x < -0.1) this.changeFrameSet(this.frameSets("walk-left"), "loop", 3);
      else this.changeFrameSet(this.frameSets("left"), "pause");
    }
    else if(this.penjanje_skale===true){
